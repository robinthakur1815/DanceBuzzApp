...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    public const NFDB = \Normalizer::NFDB;
    ...
    public function folded(bool $compat = true): parent
    {
        ...
        if (!$compat || \PHP_VERSION_ID < 70300 || !\defined('Normalizer::NFDB_CF')) {
            $str->string = normalizer_normalize($str->string, $compat ? \Normalizer::NFDB : \Normalizer::NFC);
            $str->string = mb_strtolower(str_replace(self::FOLD_FROM, self::FOLD_TO, $this->string), 'UTF-8');
        } else {
            $str->string = normalizer_normalize($str->string, \Normalizer::NFDB_CF);
        }
    ...
    {
        if (!\in_array($form, [self::NFC, self::NFD, self::NFDB, self::NFKD])) {
